# uninstall.ps1 - Remove Frappe ERPNext
$WSL = "wsl.exe"
...
schtasks /delete /tn "FrappeERPNext" /f 2>$null
...
netsh advfirewall firewall delete rule name="Frappe-ERPNext" 2>$null

Write-Host "Cleaning up WSL files..."
& $WSL -u root -- rm -rf $LMS_DIR

Write-Host "Frappe ERPNext uninstalled." -ForegroundColor Yellow
