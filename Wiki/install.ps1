# install.ps1 - Install Frappe Wiki on Windows via WSL2 + Podman
...
$confFile = Join-Path $scriptDir "wiki.conf"
...
Write-Host "=== Installing Frappe Wiki ===" -ForegroundColor Cyan
...
if ($wslVersion -match "VERSION\s+1" -and $wslVersion -notmatch "VERSION\s+2") {
    ...
    Write-Host "Frappe Wiki requires WSL2 (needs hardware virtualization / Hyper-V)."
...
& "$sharedDir\service-setup.ps1" -TaskName "FrappeWiki" -ServiceScript "$scriptDir\wiki-service.ps1"
...
# Step 5: Firewall rule (port proxy is handled by wiki-service.ps1 on every boot)
netsh advfirewall firewall add rule name="Frappe-Wiki" dir=in action=allow protocol=TCP localport=$($conf.LMS_PORT)

Write-Host ""
Write-Host "=== Frappe Wiki installed! ===" -ForegroundColor Green
Write-Host "Access at: http://wiki.localhost:$($conf.LMS_PORT)/wiki/"
Write-Host "Or from LAN: http://$($env:COMPUTERNAME):$($conf.LMS_PORT)/wiki/"
Write-Host ""
Write-Host "Task: FrappeWiki (auto-starts on boot, keeps WSL alive)"
